"""
Script để train VideoMAE model cho action recognition.
Implementation chuẩn, clean, dựa trên best practices.
"""
...
def get_videomae_config():
    ...
    # Training parameters
    config['batch_size'] = 8  # Giảm xuống 8 để tránh OOM khi unfreeze (nhiều layers trainable)
    config['grad_accum_steps'] = 4  # Tăng grad accumulation để giữ effective batch size = 32
    config['epochs'] = 100
    config['warmup_epochs'] = 5
    config['cosine_start_epoch'] = 5
    
    # Learning rates - 2-phase training strategy
    # Phase 1 (an toàn, dễ sống): freeze backbone + train head
    config['base_lr'] = 0.0  # Backbone frozen, không train
    config['head_lr'] = 1e-3  # Head LR: 5e-4 ~ 1e-3 (dùng 1e-3)
    
    # Phase 2 (tăng acc): partial unfreeze last 4-6 blocks
    config['base_lr_unfreeze'] = 1e-6  # Giảm LR xuống 1e-6 (thấp hơn để tránh phá pretrained features)
    config['head_lr_unfreeze'] = 5e-5  # Giảm head LR xuống 5e-5
    
    # Freeze strategy
    config['freeze_backbone_epochs'] = 10  # Phase 1: 10 epochs freeze backbone
    config['unfreeze_partial'] = True  # Phase 2: partial unfreeze
    config['unfreeze_num_blocks'] = 2  # Chỉ unfreeze 2 layers cuối (layers 22-23 trong VideoMAE-large 24 layers)
    config['skip_unfreeze'] = False  # Enable Phase 2 unfreeze
    
    # Regularization - Tăng để chống overfit
    config['weight_decay'] = 0.1  # Tăng weight decay
    config['dropout'] = 0.2  # Tăng dropout
    config['drop_path_rate'] = 0.1  # Bật drop path
    config['label_smoothing'] = 0.1  # Bật label smoothing
    
    # Augmentation - Bật để chống overfit
    config['mixup_alpha'] = 0.2  # Bật Mixup với alpha nhỏ
    config['cutmix_alpha'] = 1.0  # Bật CutMix
    config['use_cutmix'] = True
    config['use_temporal_aug'] = True  # Bật temporal augmentation
    config['use_advanced_spatial'] = True  # Bật advanced spatial
    config['use_advanced_color'] = True  # Bật advanced color
    ...
    config['use_adapters'] = False  # Tắt adapters để tăng tốc (forward qua từng layer thủ công rất chậm)
...
def parse_args():
    ...
    parser.add_argument('--model-id', type=int, default=7, help='Model ID')
...
def main():
    ...
    logger_instance.info("VideoMAE Training")
    logger_instance.info("="*60)
