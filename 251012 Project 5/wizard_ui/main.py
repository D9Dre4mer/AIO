...
def render_navigation_buttons():
    ...
    with col1:
        current_step = get_current_step(session_manager)
        if st.button("◀ Previous", key=f"prev_btn_{current_step}"):
    ...
    with col2:
        if st.button("Next ▶", key=f"next_btn_{current_step}"):
            # Use global session_manager instance
            current_step = get_current_step(session_manager)
...
def render_single_input_section():
    ...
    if st.button("💾 Save Column Configuration", type="primary"):
...
def render_step3_wireframe():
    ...
    with col2:
        if st.button("✅ Complete Step 3", key="complete_step3"):
